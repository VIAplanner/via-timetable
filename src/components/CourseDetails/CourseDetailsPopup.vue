        ...
        <div class="w-full flex flex-col gap-6 lg:flex-row lg:gap-0">
          <div class="w-full lg:w-[60%] lg:pr-8">
            ...
            <p class="wrap-break-words">
              <span class="font-medium">Department: </span>{{ courseData.department.name }}
            </p>
            <!-- Faculty -->
            <p class="wrap-break-word">
              <span class="font-medium">Faculty: </span>{{ courseData.faculty.name }}
            </p>
          </div>
...
const selectedLec = ref<string | null>(null)
const selectedTut = ref<string | null>(null)
const selectedPra = ref<string | null>(null)
 ...
 */
function parseSession(sessions: string[]): string {
  return sessions
    .map((session: string) => {
      ...
      const sessionName =
        session.length === 6
          ? session.substring(6) === FIRST_SEM
            ? 'Summer First Subsession'
            : 'Summer Second Subsession'
          : month === '9'
            ? 'Fall'
            : 'Winter'
 ...
 */
function parseDistributionRequirements(distributionRequirements: Breadth[]) {
  return distributionRequirements
    ...
    .filter((requirement: string) => requirement)
    .join(', ')
}
