...
export default {
  ...
  methods: {
    ...
    createDate(semester, weekday, time) {
      ...
      return new Date(year, month, startingDay + weekdayToDayIndex[weekday], time / 3600); 
    },
    saveTimetable(timetable, semester) {
      const until = semester === "F" ? new Date(2021, 11, 8) : new Date(2022, 3, 9);
      Object.entries(timetable).forEach((day) => {
        const weekday = day[0], sections = day[1];
        sections.forEach((section) => { 
          const start = this.createDate(semester, weekday, section.start);
          const end = this.createDate(semester, weekday, section.end);
          this.$ics.addEvent("en-us", `${section.code} ${section.sectionCode}`, "", section.location, start, end, "", null, {
            freq: 'WEEKLY',
            until,
            interval: 1
          });
