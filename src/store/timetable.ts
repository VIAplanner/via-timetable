...
const timetableActions: ThisType<TimetableStore> & Record<string, (...args: any[]) => any> = {
  ...
  setBlockedTime(semester: SemesterCode, day: Weekday, start: number, end: number, block: boolean) {
    if (block) {
      ...
      if (
        !this.blockedTimes[semester].some(
      ...
      this.blockedTimes[semester] = this.blockedTimes[semester].filter(
        (blocker) => !(blocker.day === day && blocker.start === start && blocker.end === end),
      )
  ...
  async addCourse(
    ...
    const selectedSessionFromCourse =
      this.resolveSubsessionSemesters(courseData.sessions?.[0])?.[0] || null
  ...
  async addCourseToBuilder(courseData: any) {
    ...
    for (const sectionData of courseData['sections']) {
      ...
      for (const meetingTimeData of meetingTimes) {
        ...
        const fallbackSemesters = this.resolveSubsessionSemesters(
        ...
        for (const semesterIndex of semesterIndexes) {
          ...
          const mode = this.getCourseSectionDeliveryModeForSession(
  ...
  normalizeBuiltTimetable(timetable) {
    ...
    for (const entry of timetable) {
      ...
      const matchingCandidateSessions = candidateSessions.filter(
        (session): session is SemesterCode => {
      ...
      if (matchingCandidateSessions.length > 1) {
        ...
        const matchedSessions = matchingCandidateSessions.filter((session) => {
          ...
          return meetingTimes.some((meetingTime: any) =>
            this.resolveSubsessionSemesters(meetingTime.sessionCode).includes(session),
          )
        })
  ...
  async getDivisionalEnrolmentIndicators(): Promise<any> {
    if (
      ...
      this.divisionalEnrolmentIndicators.expiry < Date.now()
    ) {
      try {
  ...
  timetableRegisterActivity(courseData, activityName) {
    ...
    if (activity) {
      for (const meetingTime of activity.meetingTimes) {
        ...
        if (!selectedSessionFromActivity && sessions.length > 0)
          selectedSessionFromActivity = sessions[0] as SemesterCode

        for (const session of sessions as Array<SemesterCode>) {
          // Add events that are not already present
          if (
            !this.timetables[session][day].some(
              (timeslot) =>
      ...
      if (this.switchSession && selectedSessionFromActivity)
        this.selectedSession = selectedSessionFromActivity
    }
  },
  ...
  async timetableModifyActivity(courseData, newActivityName, lockAndGenerate = false) {
    ...
    const courseExists = SEMESTER_CODES.some((session) =>
      Object.prototype.hasOwnProperty.call(this.selectedCourses[session], courseData.code),
    )
  ...
  async updatePreferences() {
    ...
    manager.setPreferences({
      ...
      ONLINE_PREFERENCE:
        this.onlinePreference === 'Avoid' ? 0 : this.onlinePreference === 'Prefer' ? 1 : 2,
    })
  },
